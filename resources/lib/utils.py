...
def isLoggedIn(func):
    ...
    def login_wrapper(*args, **kwargs):
        ...
        if headers and exp > time.time():
            ...
            executebuiltin(
                "RunPlugin(plugin://kodi.repo.jiotv/resources/lib/main/login/)")
            return False
        else:
            Script.notify(
...
def login(username, password, mode="unpw"):
    ...
    resp = urlquick.post("https://api.jio.com/v3/dip/user/{0}/verify".format(mode), json=body, headers={
                         "User-Agent": "JioTV", "x-api-key": "l7xx938b6684ee9e4bbe8831a9a682b8e19f"}, max_age=-1, verify=False, raise_for_status=False).json()
...
def sendOTP(mobile):
    ...
    resp = urlquick.post("https://api.jio.com/v3/dip/user/otp/send", json=body, headers={
        "x-api-key": "l7xx938b6684ee9e4bbe8831a9a682b8e19f"}, max_age=-1, verify=False, raise_for_status=False)
